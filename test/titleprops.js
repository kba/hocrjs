...
test("PropertyParser", function (t) {
    'use strict';

    var pp = new PropertyParser({ allowUnknown: true, debug: process.env.DEBUG });
    var mergeFixtures = function mergeFixtures(names) {
        var fixture = [names.map(function (name) {
            return fixtures[name][0];
        }).join(';'), fixtures[names[0]][1], {}];
        for (var i = 0; i < names.length; i++) {
            var name = names[i];
            if (i > 0) fixture[1] = fixture[1].concat(';').concat(fixtures[name][1]);
            for (var k in fixtures[name][2]) {
                fixture[2][k] = fixtures[name][2][k];
            }
        }
        return fixture;
    };
